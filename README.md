...
- `data/` 原始处理数据；
- `reports/` 本地 PDF；本地 PDF 只保留私有仓库点击地址，不进入公开站点；
- 本地绝对路径、Obsidian wikilink/callout 和高置信度密钥内容。

## 完整性保证

- Markdown：源文件与公开副本按顺序核对可见文本和完整表格矩阵；
- 图片：源文件与公开副本核对字节数和 SHA-256；页面内仍使用公开构建资产显示原图，点击地址固定到私有仓库原文件；
- CSV：源文件与公开副本核对字节数和 SHA-256，并按完整性清单原字节复制进最终 Pages 目录；
- 渲染页面：再次核对正文块、表格单元格、显式外部链接、图片点击目标和全部 CSV 构建产物；
- 清单：`site/public/research-manifest.json` 记录全部公开文件、哈希、表格维度、私有仓库上下文和附件链接改写。

### 附件链接

研究正文中的图片会作为经过校验的 GitHub Pages 资产公开，使文章本身保持可读。点击 vault 本地的图片、SVG、CSV 或 PDF 时，统一打开私有 `teazean/obsidian-vault-invest` 仓库中对应的 `blob/master` 地址；读者必须登录有该仓库权限的 GitHub 账号。正文中原本直接指向公告、公司官网等 HTTP(S) 权威来源的链接保持不变。
...
本地 PDF 仍不会进入公开站点。所有 vault 本地附件的点击目标都改写为私有仓库的 `github.com/.../blob/master/...` 页面；正文原有的 HTTP(S) 权威链接不改写。私有链接只对已登录且拥有该仓库权限的用户可见，公开页面会暴露仓库名和文件路径，但不会公开 PDF 内容。
